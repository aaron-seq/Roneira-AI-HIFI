...
export function LandingTicker() {
  const { data, isLoading, isError } = useMarketOverviewData();
  ...
  if (isError) {
    return (
      <div
        className="border-y py-3 text-center text-xs"
        style={{ borderColor: "var(--color-divider)", background: "var(--color-surface)", color: "var(--color-text-muted)" }}
      >
  ...
  function renderRow(copy: "a" | "b") {
    return quotes.map((quote) => (
      <span key={`${quote!.symbol}-${copy}`} className="flex shrink-0 items-baseline gap-2 px-6 text-sm">
