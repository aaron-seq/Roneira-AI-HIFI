...
export function CommandPalette() {
  ...
  return (
    <AnimatePresence>
      {open && <Palette key="palette" onClose={() => setOpen(false)} />}
    </AnimatePresence>
  );
}

// Query and selection live here rather than in CommandPalette so that closing
// the palette unmounts them: they come back empty on the next open by
// construction, instead of being reset by an effect that watched `open`.
function Palette({ onClose }: { onClose: () => void }) {
  ...
  // Arrow keys + Enter. A plain function, not useCallback: the input it feeds is
  // a DOM element, so there was no memoized child for a stable identity to help.
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-4" style={{ borderBottom: "1px solid var(--color-divider)" }}>
          ...
          {/* autoFocus replaces a focus-on-open effect: the palette is a modal
              search box, so mount is exactly when it should take focus. */}
          <input
            autoFocus
            type="text"
            value={query}
