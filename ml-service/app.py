...
class StockDataProcessor:
    ...
    @staticmethod
    def fetch_historical_stock_data(ticker_symbol: str, 
                                  start_date: str, 
    ...
    def calculate_technical_indicators(stock_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            stock_data['simple_moving_average_5'] = stock_data['Close'].rolling(window=5).mean()
            stock_data['simple_moving_average_20'] = stock_data['Close'].rolling(window=20).mean()
            stock_data['simple_moving_average_50'] = stock_data['Close'].rolling(window=50).mean()
            
            # Exponential Moving Averages
            stock_data['exponential_moving_average_12'] = stock_data['Close'].ewm(span=12).mean()
            stock_data['exponential_moving_average_26'] = stock_data['Close'].ewm(span=26).mean()
            
            # RSI (Relative Strength Index)
            price_delta = stock_data['Close'].diff()
            price_gains = (price_delta.where(price_delta > 0, 0)).rolling(window=14).mean()
            price_losses = (-price_delta.where(price_delta < 0, 0)).rolling(window=14).mean()
            relative_strength = price_gains / price_losses
            stock_data['relative_strength_index'] = 100 - (100 / (1 + relative_strength))
            
            # MACD (Moving Average Convergence Divergence)
            macd_line = stock_data['exponential_moving_average_12'] - stock_data['exponential_moving_average_26']
            ...
            # Volume indicators
            stock_data['volume_moving_average_20'] = stock_data['Volume'].rolling(window=20).mean()
            stock_data['volume_ratio'] = stock_data['Volume'] / stock_data['volume_moving_average_20']
...
class SentimentAnalysisService:
    ...
    def __init__(self, hugging_face_api_key: str):
        ...
        self.finbert_sentiment_url = "https://api-inference.huggingface.co/models/ProsusAI/finbert"
        
    def analyze_stock_sentiment(self, ticker_symbol: str, company_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Sentiment analysis API returned status code: {response.status_code}")
...
class MachineLearningModelTrainer:
    ...
    def __init__(self):
        self.supported_features = [
            ...
            'simple_moving_average_5', 'simple_moving_average_20', 'simple_moving_average_50',
            'relative_strength_index', 'macd_line', 'macd_signal_line',
            'bollinger_band_upper', 'bollinger_band_lower', 'bollinger_position',
            'volume_ratio'
        ]
    ...
    def prepare_training_features(self, stock_data: pd.DataFrame) -> tuple:
        ...
        available_features = [feature for feature in self.supported_features 
    ...
    def train_random_forest_model(self, ticker_symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            start_date = end_date - timedelta(days=app_config.model_training_lookback_days)
            ...
            # Prepare features
            feature_matrix, target_prices, available_features = self.prepare_training_features(stock_data)
            ...
            logger.info(f"Model trained for {ticker_symbol} - R2 Score: {test_r2_score:.4f}, RMSE: {test_rmse:.4f}")
    ...
    try:
        ...
        if not ticker_symbol:
            return jsonify({
                ...
                'example_request': {'ticker': 'AAPL', 'days': 1, 'include_pdm': True}
            }), 400
        ...
        if cache_key in prediction_results_cache:
            ...
            if (datetime.now() - cache_timestamp).seconds < app_config.prediction_cache_timeout_seconds:
        ...
        recent_stock_data = StockDataProcessor.calculate_technical_indicators(recent_stock_data)
        ...
        latest_features = clean_recent_data[model_info['feature_names']].iloc[-1].values.reshape(1, -1)
        ...
        market_sentiment = sentiment_analyzer.analyze_stock_sentiment(ticker_symbol, company_name)
        ...
        prediction_response = {
            ...
            'timestamp': datetime.now().isoformat(),
            'technical_indicators': {
                'relative_strength_index': round(float(clean_recent_data['relative_strength_index'].iloc[-1]), 2) if 'relative_strength_index' in clean_recent_data.columns else None,
                'simple_moving_average_5': round(float(clean_recent_data['simple_moving_average_5'].iloc[-1]), 2) if 'simple_moving_average_5' in clean_recent_data.columns else None,
                'simple_moving_average_20': round(float(clean_recent_data['simple_moving_average_20'].iloc[-1]), 2) if 'simple_moving_average_20' in clean_recent_data.columns else None,
                'macd_line': round(float(clean_recent_data['macd_line'].iloc[-1]), 2) if 'macd_line' in clean_recent_data.columns else None,
                'bollinger_position': round(float(clean_recent_data['bollinger_position'].iloc[-1]), 2) if 'bollinger_position' in clean_recent_data.columns else None
            }
        }
            ...
            try:
                ...
                if pdm_signal:
                    prediction_response['pdm_strategy_analysis'] = {
                        ...
                        'institutional_volume_factor': round(pdm_signal.institutional_volume_factor, 3),
        ...
        logger.info(f"Prediction completed for {ticker_symbol}: ML={ml_predicted_price:.2f}")
    ...
    try:
        ...
        for signal in pdm_opportunities:
            formatted_opportunities.append({
                ...
                'institutional_factor': round(signal.institutional_volume_factor, 3),
    ...
    try:
        ...
        if not ticker_list or len(ticker_list) == 0:
            return jsonify({
                ...
                'example_request': {'tickers': ['AAPL', 'GOOGL', 'MSFT'], 'include_pdm': False}
            }), 400
...
def execute_pdm_backtest():
    ...
    try:
        ...
        backtest_results = pdm_strategy_engine.backtest_pdm_strategy(start_date, end_date)
