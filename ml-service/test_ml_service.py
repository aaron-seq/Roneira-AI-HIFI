...
class TestPDMStrategyEngine:
    ...
    def setup_method(self):
        ...
        self.pdm_engine = PriceVolumeDerivativesEngine(
            lookback_period=100, min_liquidity=500_000
        )
    ...
    def test_price_curvature_calculation(self):
        ...
        assert not np.isnan(
            curvature.iloc[2]
        )  # Third point should have valid curvature
    ...
    def test_volume_sensitivity_calculation(self):
        ...
        volume_sensitivity = self.pdm_engine.calculate_volume_sensitivity(
            price_series, volume_series
        )

        assert len(volume_sensitivity) == len(price_series)
        assert not np.isinf(
            volume_sensitivity.iloc[-1]
        )  # Should handle division properly
    ...
    def test_atr_calculation(self):
        ...
        atr = self.pdm_engine.calculate_average_true_range(
    ...
    def test_institutional_volume_detection(self):
        ...
        institutional_factor = (
            self.pdm_engine.detect_institutional_volume_participation(
    ...
    def test_liquidity_filtering(self):
        ...
        with patch("yfinance.download") as mock_download:
            ...
            mock_download.side_effect = [
                pd.DataFrame({"Close": [150] * 30, "Volume": [2_000_000] * 30}),
...
class TestFlaskApplication:
    ...
    def test_predict_endpoint_missing_ticker(self):
        ...
        response = self.client.post(
            "/predict", json={}, content_type="application/json"
        )
