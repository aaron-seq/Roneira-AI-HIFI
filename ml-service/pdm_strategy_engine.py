...
class PriceVolumeDerivativesEngine:
    ...
    def generate_pdm_signals(self, symbol: str) -> Optional[PDMSignal]:
        ...
        try:
            ...
            # PDM Entry Logic
            trend_confirmation = (
                moving_averages["sma_20"].iloc[-1] > moving_averages["sma_200"].iloc[-1]
                and latest_price > moving_averages["sma_20"].iloc[-1]
            )

            positive_velocity = latest_velocity > 0
            early_impulse = latest_curvature < 0  # Momentum peak capture
            volume_validation = (
                latest_institutional_factor > 1.2
            ...
            confidence_components = [
                ...
                min(
                    latest_institutional_factor / 2.0, 1.0
                ),  # Normalize institutional factor
            ]
            ...
            trailing_stop = latest_price - (
                latest_atr * self.atr_trailing_stop_multiplier
            )
