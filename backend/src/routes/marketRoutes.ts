...
const fetchStockQuote = async (symbol: string): Promise<StockQuote | null> => {
  ...
  try {
    ...
    logger.error(`Error fetching quote for ${symbol}: ${error instanceof Error ? error.message : JSON.stringify(error)}`);
    return null;
  }
};
...
router.get('/overview', async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.error(`Error in market overview: ${error instanceof Error ? error.message : JSON.stringify(error)}`);
    res.status(500).json({
      success: false,
...
router.get('/quote/:symbol', async (req: Request, res: Response) => {
  try {
    ...
    logger.error(`Error fetching quote: ${error instanceof Error ? error.message : JSON.stringify(error)}`);
    res.status(500).json({
      success: false,
...
router.get('/movers', async (req: Request, res: Response) => {
  try {
    ...
    logger.error(`Error fetching market movers: ${error instanceof Error ? error.message : JSON.stringify(error)}`);
    res.status(500).json({
      success: false,
