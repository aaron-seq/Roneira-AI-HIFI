 ...
 */
export async function getBatchQuotes(symbols: string[]): Promise<Record<string, GlobalQuote | null>> {
 ...
 */
export async function getTimeSeriesDaily(symbol: string, outputSize: 'compact' | 'full' = 'compact'): Promise<TimeSeriesData | null> {
    ...
    try {
        ...
        const data: TimeSeriesData = {
            symbol: symbol,
            data: Object.entries(timeSeriesData).map(([date, values]: [string, any]) => ({
                date,
                open: values['1. open'],
                high: values['2. high'],
                low: values['3. low'],
...
export async function getMarketOverview(): Promise<any> {
    ...
    try {
        ...
        const indices = [
            { name: 'S&P 500 (SPY)', value: quotes['SPY']?.price || 'N/A', change: quotes['SPY']?.change_percent || 'N/A' },
            { name: 'Dow 30 (DIA)', value: quotes['DIA']?.price || 'N/A', change: quotes['DIA']?.change_percent || 'N/A' },
            { name: 'Nasdaq (QQQ)', value: quotes['QQQ']?.price || 'N/A', change: quotes['QQQ']?.change_percent || 'N/A' },
            { name: 'Russell 2000 (IWM)', value: quotes['IWM']?.price || 'N/A', change: quotes['IWM']?.change_percent || 'N/A' }
        ];

        const topPerformersData = topPerformers.map(ticker => ({
            ticker,
